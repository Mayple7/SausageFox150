...
Enemy *CreateEnemy(int enemyType, int collisionGroup, float xPos, float yPos, int panelId)
{
	...
	case BasicMelee:
		...
		InitializeEnemyStats(CurrentEnemy, 50 + 10 * (rand() % GetCurrentState()), (float)(300 + 10 * (rand() % 10)), 8.0f, 0, 20 + 6 * (rand() % GetCurrentState()), 10 + rand() % 10, 53);

		// Special state for level 3.1
		if(GetCurrentState() == GS_Level31)
		{
			CurrentEnemy->CurrentEnemyStats.Damage = 20 + 3 * (rand() % 4);
			CurrentEnemy->CurrentEnemyStats.MaxHealth = 50 + 10 * (rand() % 4);
